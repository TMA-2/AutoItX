...
public class Wait_AU3WinNotActive : PSCmdlet
{
    ...
    protected override void ProcessRecord()
    {
        ...
        if (!(parameterSetName == "Text"))
        {
            ...
            {
                ((Cmdlet)this).WriteObject((object)AutoItX.WinWaitNotActive(winHandle, timeout), true);
            }
        }
        else
        {
            ((Cmdlet)this).WriteObject((object)AutoItX.WinWaitNotActive(title, text, timeout), true);
        }
    }
}
